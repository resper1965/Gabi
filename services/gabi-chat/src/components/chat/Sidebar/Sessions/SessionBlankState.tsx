  ...
  const errorMessage = (() => {
    switch (true) {
      case !isEndpointActive:
        return 'Endpoint não conectado. Conecte o endpoint para ver o histórico.'
      case !selectedEndpoint:
        return 'Selecione um endpoint para ver o histórico.'
      case !agentId:
        return 'Selecione um agente para ver o histórico.'
      default:
        return 'Nenhum registro de sessão ainda. Inicie uma conversa para criar um.'
    }
  })()

  return (
    <div className="mt-1 flex items-center justify-center rounded-lg bg-secondary/30 border border-primary/10 pb-6 pt-4">
      <div className="flex flex-col items-center gap-3">
        <HistoryBlankStateIcon />
        <div className="flex flex-col items-center gap-2">
          <h3 className="text-sm font-montserrat font-medium text-white">Nenhuma Sessão Encontrada</h3>
          <p className="max-w-[210px] text-center text-xs font-montserrat text-primary/80">
            {errorMessage}
          </p>
        </div>
      </div>
