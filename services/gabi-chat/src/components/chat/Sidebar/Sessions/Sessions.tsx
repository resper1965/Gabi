...
const Sessions = () => {
  ...
  // Carregar sessões do localStorage como fallback
  useEffect(() => {
    if (hydrated && (!isEndpointActive || sessionsData?.length === 0)) {
      try {
        const localSessions = JSON.parse(localStorage.getItem('gabi_sessions') || '[]')
        if (localSessions.length > 0) {
          console.log('Carregando sessões do localStorage:', localSessions.length)
          setSessionsData(localSessions)
        }
      } catch (error) {
        console.error('Erro ao carregar sessões do localStorage:', error)
      }
    }
  }, [hydrated, isEndpointActive, sessionsData?.length, setSessionsData])

  // Escutar evento de sessão salva para atualizar a lista
  useEffect(() => {
    const handleSessionSaved = () => {
      console.log('Evento de sessão salva recebido, recarregando lista...')
      try {
        const localSessions = JSON.parse(localStorage.getItem('gabi_sessions') || '[]')
        console.log('Recarregando sessões:', localSessions.length)
        setSessionsData(localSessions)
      } catch (error) {
  ...
  if (isSessionsLoading || isEndpointLoading) {
    return (
      <div className="w-full">
        <div className="mb-2 text-xs font-medium uppercase text-white">Sessions</div>
  ...
  return (
    <div className="w-full">
      <div className="mb-2 w-full text-xs font-medium uppercase text-white">Sessions</div>
      <div
