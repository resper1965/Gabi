"""
ANEEL Ingestion — Agência Nacional de Energia Elétrica
...
SOURCE = IngestSource.ANEEL_NORM


async def run_aneel_ingestion(days: int = 30):
    """Ingest latest ANEEL normative acts from the DOU."""
    print(f"[*] Iniciando Ingestão de Normativos da ANEEL (Últimos {days} dias)")
    dou = DOUClient()
    ...
    async with async_session() as db:
        ...
        docs = await dou.buscar_por_orgao("ANEEL", dias=days)
        print(f"[*] Encontrados {len(docs)} documentos ANEEL no DOU.")
