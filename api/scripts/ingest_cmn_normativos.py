"""
CMN Normatives Ingestion — Conselho Monetário Nacional
Uses the official BCB OData API (olinda.bcb.gov.br) with tipo_filter='Resolução CMN'.
"""
...
async def run_ingestion(days: int = 90):
    print(f"[*] Iniciando Ingestão de Normativos do CMN (últimos {days} dias) — via Olinda OData")
    client = OlindaClient()
