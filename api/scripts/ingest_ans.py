"""
ANS Ingestion — Agência Nacional de Saúde Suplementar
...
AUTHORITY = RegulatoryAuthority.ANS
SOURCE = IngestSource.ANS_NORM


async def run_ans_ingestion(days: int = 30):
    """Ingest latest ANS normative acts from the DOU."""
    print(f"[*] Iniciando Ingestão de Normativos da ANS (Últimos {days} dias)")
    dou = DOUClient()
    ...
    async with async_session() as db:
        ...
        docs = await dou.buscar_por_orgao("ANS", dias=days)
        print(f"[*] Encontrados {len(docs)} documentos ANS no DOU.")
