"""
ANPD Ingestion — Autoridade Nacional de Proteção de Dados
...
SOURCE = IngestSource.ANPD_NORM


async def run_anpd_ingestion(days: int = 30):
    """Ingest latest ANPD normative acts from the DOU."""
    print(f"[*] Iniciando Ingestão de Normativos da ANPD (Últimos {days} dias)")
    dou = DOUClient()
    ...
    async with async_session() as db:
        ...
        docs = await dou.buscar_por_orgao("ANPD", dias=days)
        print(f"[*] Encontrados {len(docs)} documentos ANPD no DOU.")
