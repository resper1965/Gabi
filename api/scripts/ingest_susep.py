"""
SUSEP Ingestion — Superintendência de Seguros Privados
...
SOURCE = IngestSource.SUSEP_NORM


async def run_susep_ingestion(days: int = 30):
    """Ingest latest SUSEP normative acts from the DOU."""
    print(f"[*] Iniciando Ingestão de Normativos da SUSEP (Últimos {days} dias)")
    dou = DOUClient()
    ...
    async with async_session() as db:
        ...
        docs = await dou.buscar_por_orgao("SUSEP", dias=days)
        print(f"[*] Encontrados {len(docs)} documentos SUSEP no DOU.")
