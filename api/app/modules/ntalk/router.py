...
async def ask_gabi_stream(
    ...
    SSE protocol:
      data: {"type": "meta", "interpretation": "...", "sql": "...", "results": {...}}  ← first
    ...
    # Steps 1-5: same as /ask (sync pipeline)
    emb = await asyncio.to_thread(embed, req.question)
    ...
    # Step 6: Stream CFO interpretation
    async def _stream():
        yield f"data: {meta}\n\n"

        if results and results.get("rows"):
            preview = json.dumps(results["rows"][:20], default=str, ensure_ascii=False)
            cfo_prompt = f"Pergunta: {req.question}\nSQL: {generated_sql}\nDados:\n{preview}"
            cfo_system = """[PERSONA] Você é a Gabi, CFO Sênior do mercado imobiliário.
[RESTRIÇÕES]
1. Interprete SOMENTE os dados retornados pela query acima.
2. NÃO invente dados, tendências ou benchmarks de mercado não presentes nos resultados.
3. Se os dados forem insuficientes: "Os dados retornados não permitem esta análise."
